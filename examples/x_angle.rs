...
fn main() -> ! {
    ...
    let mut x_angle_min: f32 = 91.0;
    let mut x_angle_max: f32 = -91.0;
    ...
    loop {
        ...
        if (fifodata[0] & 0x80) == 0x80 {
            ...
            if count == AVERAGE_NUMBER {
                ...
                let x_average = x_average as f32 / AVERAGE_NUMBER as f32;
                // let y_average = y_average as f32 / AVERAGE_NUMBER as f32;
                // let z_average = z_average as f32 / AVERAGE_NUMBER as f32;

                let x_avarage_a = K*asinf(x_average/16_384.0);

                if x_avarage_a > x_angle_max {
                    x_angle_max = x_avarage_a;
                }

                if x_avarage_a < x_angle_min {
                    x_angle_min = x_avarage_a;
                }
                ...
                println!("x_angle: {:.3} {:.3} {:.3} {:.3}", x_avarage_a, 
                                                             x_angle_min, 
                                                             x_angle_max, 
                                                             x_angle_max-x_angle_min);
            } else {
